...
const RELAY_POLL_TIMEOUT = 20 * 60 * 1000; // 20 minutes — accommodates slow NPO websites
...
export default function Home() {
  ...
  async function handleProcess() {
    ...
    // Scraper accepts the request and runs in background — long scrapes can
    // take 10+ minutes, exceeding HTTP timeouts. Final results land in the Sheet.
    scraperPromise
      .then(data => {
        setScraperStatus(S.QUEUED);
        setScraperResult(data);
      })
      .catch(e => { setScraperStatus(S.ERROR); setScraperResult({ error: e.message }); });

    // When Relay trigger succeeds, capture runLink, switch to QUEUED, start polling
    relayPromise
      .then(triggerData => {
        // triggerData includes { status: "triggered", runId, runLink } from /api/relay
        setRelayStatus(S.QUEUED);
        setRelayResult(triggerData);
        const startedAt = Date.now();

        relayPollRef.current = setInterval(async () => {
          // Give up after timeout — keep runLink visible so user can still inspect the run
          if (Date.now() - startedAt > RELAY_POLL_TIMEOUT) {
            clearInterval(relayPollRef.current);
            setRelayStatus(S.ERROR);
            setRelayResult(prev => ({ ...prev, error: "Timed out waiting for Relay to complete" }));
            return;
          }
          try {
            ...
            if (data.done) {
              ...
              // Merge callback data onto trigger data so runLink survives
              setRelayResult(prev => ({ ...prev, ...data }));
            }
          } catch (e) {
        ...
        {(scraperStatus !== S.IDLE || relayStatus !== S.IDLE) && (
          <div style={{
            ...
            <div style={{ padding: "14px 16px", borderBottom: "1px solid #2d2d38" }}>
              ...
              {scraperStatus === S.QUEUED && (
                <div style={{ marginTop: 8, fontSize: 12, color: "#f59e0b" }}>
                  Scraper running in background — images will appear in the Drive folder, and the result will be written to the Google Sheet when complete (typically 2–15 min).
                </div>
              )}
            ...
            <div style={{ padding: "14px 16px" }}>
              ...
              {/* Run link — visible whenever we have one (queued/done/warn/error after trigger) */}
              {relayResult && relayResult.runLink && relayStatus !== S.IDLE && relayStatus !== S.RUNNING && (
                <a href={relayResult.runLink} target="_blank" rel="noreferrer"
                  style={{

...
export default function Home() {
  ...
  async function handleProcess() {
    ...
    relayPromise
      .then(data => { setRelayStatus(S.DONE); setRelayResult(data); })
      .catch(e => { setRelayStatus(S.ERROR); setRelayResult({ error: e.message }); });
  }

  const statusIcon = s => s === S.IDLE ? "—" : s === S.RUNNING ? "⏳" : s === S.DONE ? "✅" : "❌";
  const statusColor = s => s === S.RUNNING ? "#a78bfa" : s === S.DONE ? "#34d399" : s === S.ERROR ? "#f87171" : "#6b7280";
            ...
            <div style={{ padding: "14px 16px" }}>
              ...
              {relayStatus === S.DONE && relayResult && (
                <div style={{ marginTop: 8, fontSize: 12, color: "#9ca3af" }}>
                  <div>Workflow triggered — social handles will be written to the Sheet shortly.</div>
                  {relayResult.runLink && (
